...
import type { RootState, Student } from './types';
...
const AdminProtectedRoute: React.FC<AdminProtectedRouteProps> = ({ element, adminIsAuthenticated, adminRole }) => {
...
const SuperAdminProtectedRoute: React.FC<AdminProtectedRouteProps> = ({ element, adminIsAuthenticated, adminRole }) => {
  return adminIsAuthenticated && (adminRole === 'superadmin' || adminRole === 'SUPERADMIN') ? element : <Navigate to="/admin/login" />;
};

function App() {
  const { isAuthenticated } = useSelector((state: RootState) => state.auth);
  const { isAuthenticated: adminIsAuthenticated, role: adminRole } = useSelector((state: RootState) => state.adminAuth);
  const dispatch = useDispatch();

  useEffect(() => {
    if (tokenService.isStudentTokenValid()) {
      const storedUser = tokenService.getStudentUser() as Student | null;
      if (storedUser) {
