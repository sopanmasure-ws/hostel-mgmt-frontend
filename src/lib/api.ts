...
const getAuthHeaders = (token: string | null = null): Record<string, string> => {
...
const apiFetch = async <T = unknown>(endpoint: string, options: ApiFetchOptions = {}): Promise<T> => {
  const headers = getAuthHeaders(options.token || null);
...
const createRequestOptions = (method: string = 'POST', data: unknown, token: string | null = null): ApiFetchOptions => ({
  method,
...
export const applicationAPI = {
  ...
  getApplicationsByPNR: async (pnr) => {
    try {
      const resp = await apiFetch(`/applications/${pnr}`) as any;
...
export const adminRoomAPI = {
  getInventory: async (hostelId, filters: { status?: string; floor?: string | number } = {}) => {
...
export const superAdminAPI = {
  ...
  /**
   * GET /superadmin/hostels/:hostelId/applications
   * Get all applications for a specific hostel
   */
  getHostelApplications: async (hostelId) => {
    return await apiFetch(`/superadmin/hostels/${hostelId}/applications`);
  },

  /**
   * PUT /api/superadmin/applications/:applicationId/assign-room
   * Assign room to a pending application
   * Request body: { roomNumber, hostelId }
   */
  assignRoom: async (applicationId, assignData) => {
    return await apiFetch(`/superadmin/applications/${applicationId}/assign-room`, createRequestOptions('PUT', assignData));
  },
};
...
export const paginationHelper = {
   ...
   */
  buildQueryString: (filters: { search?: string; page?: string | number; limit?: string | number; status?: string; role?: string; gender?: string } = {}) => {
    const params = new URLSearchParams();
