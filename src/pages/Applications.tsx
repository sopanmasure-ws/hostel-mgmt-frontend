...
import type { Application, Hostel, NotificationContextType, RootState } from '../types';
...
const Applications: React.FC = () => {
    ...
    const fetchApplications = async () => {
      try {
        ...
        if (Array.isArray(response)) {
          applicationsData = response as Application[];
        } else if (response?.application) {
          applicationsData = Array.isArray(response.application)
  ...
  const hostelMap = useMemo(() => {
    const map: Record<string, Hostel> = {};
    hostels?.forEach((hostel) => {
      if (hostel?._id) {
        map[hostel._id] = hostel as Hostel;
      }
    });
    return map;
  }, [hostels]);
        ...
        {applications.length === 0 ? (
          ...
          <div className="space-y-6">
            {applications.map((application) => {
              const hostelId = typeof application?.hostelId === 'string' ? application.hostelId : application?.hostelId?._id;
              const hostel = hostelId ? hostelMap[hostelId] : undefined;

              return (
                <div key={application._id || application.id || hostelId} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                  <div className="bg-gradient-to-r from-primary to-secondary p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="text-white">
                      <h3 className="text-xl font-bold">{hostel?.name || application?.hostelId?.name || 'Hostel'}</h3>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          ...
                          <span className="text-sm text-gray-900">{hostel?.name || application?.hostelId?.name || 'N/A'}</span>
                        </div>
