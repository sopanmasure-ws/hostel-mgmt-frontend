...
import type { Hostel, RootState } from '../types';
...
const HostelDetails: React.FC = () => {
  ...
  const { selectedHostel, hostels } = useSelector((state: RootState) => state.hostel);

  const hostel = (selectedHostel || hostels?.find((h: Hostel) => h._id === hostelId)) as Hostel | undefined;
  ...
  const handleApply = () => {
    navigate(`${ROUTES.APPLICATION_FORM}/${hostel._id || (hostel as Hostel & { id?: string }).id}`);
  };
