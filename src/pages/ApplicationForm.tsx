...
const ApplicationForm: React.FC = () => {
  ...
  const { hostels, selectedHostel } = useSelector((state: RootState) => state.hostel);
  ...
  const hostel = useMemo(() => {
    ...
    if (hostelId) {
      found = hostels.find((h: Hostel) => h._id === hostelId) || null;
      if (!found && !Number.isNaN(Number(hostelId))) {
        found = hostels.find((h: Hostel & { id?: number }) => h.id === parseInt(hostelId, 10)) || null;
      }
    } else if (selectedHostel) {
      found = selectedHostel as Hostel;
    }
    return found;
