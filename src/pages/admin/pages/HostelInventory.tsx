...
import type { NotificationContextType, RootState, Room } from '../../../types';
...
const HostelInventory: React.FC = () => {
  ...
  const { rooms, currentHostel } = useSelector((state: RootState) => state.admin);
  const notificationContext = useContext(NotificationContext) as NotificationContextType | undefined;
  const showNotification = notificationContext?.showNotification || (() => {});

  const [loading, setLoading] = useState<boolean>(true);
  const [floorFilter, setFloorFilter] = useState<string | number | null>(null);
  const [statusFilter, setStatusFilter] = useState<string | null>(null);
  const [viewType, setViewType] = useState<'grid' | 'seatmap'>('grid'); // 'grid' or 'seatmap'
