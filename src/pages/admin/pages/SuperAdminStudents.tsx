...
const SuperAdminStudents: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const cachedData = cacheService.get(cacheKey, 'local') as { version?: string; students?: any[] } | null;
  ...
  useEffect(() => {
    const enrichStudents = async () => {
      ...
      try {
        const updates: Array<{ pnr: string; detail: any }> = await Promise.all(
          toFetch.map(async (s) => {
            const cacheKey = `superadmin_student_${s.pnr}`;
            const cached = cacheService.get(cacheKey, 'local') as any;
            if (cached) {
              return { pnr: s.pnr, detail: cached };
            }
            const resp = await superAdminAPI.getStudentByPNR(s.pnr) as any;
            const detail = apiTransformers.transformStudentDetail(resp) as any;
  ...
  useEffect(() => {
    const enrichWithApplications = async () => {
      ...
      try {
        const results: Array<{ pnr: string; app: any }> = await Promise.all(
          targets.map(async (s) => {
            try {
              ...
              const app = await applicationAPI.getApplicationsByPNR(s.pnr) as any;
  ...
  const handleAssignRoom = async (student) => {
    ...
    try {
      ...
      const response = await superAdminAPI.getStudentByPNR(student.pnr) as any;
      ...
      if (studentDetails?.currentApplication?.hostelId) {
        ...
        const resp = await adminRoomAPI.getAvailableRooms(hid) as any;
        let roomsData = [];
  ...
  const handleChangeRoom = async (student) => {
    ...
    try {
      ...
      const hostelsResponse = await superAdminAPI.getAllHostels() as any;
      let allHostels = [];
      ...
      if (currentHostelId) {
        ...
        try {
          const resp = await adminRoomAPI.getAvailableRooms(currentHostelId) as any;
          let roomsData = [];
          if (Array.isArray(resp)) roomsData = resp;
          else if (resp?.data?.rooms && Array.isArray(resp.data.rooms)) roomsData = resp.data.rooms;
          else if (resp?.rooms && Array.isArray(resp.rooms)) roomsData = resp.rooms;
  ...
  const handleHostelChangeForRoom = async (hostelId) => {
    ...
    try {
      ...
      const resp = await adminRoomAPI.getAvailableRooms(hostelId) as any;
      let roomsData = [];
      if (Array.isArray(resp)) roomsData = resp;
      else if (resp?.data?.rooms && Array.isArray(resp.data.rooms)) roomsData = resp.data.rooms;
      else if (resp?.rooms && Array.isArray(resp.rooms)) roomsData = resp.rooms;
